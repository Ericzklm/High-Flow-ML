...
inputTrain, inputTest, outputTrain, outputTest = train_test_split(inputData, outputData, test_size = .3, random_state = 24)
...
trainMatrix = xgb.DMatrix(inputTrain, label=outputTrain, feature_names=inputColumns[:42])
testMatrix = xgb.DMatrix(inputTest, label=outputTest, feature_names=inputColumns[:42])
...
model.save_model('../covid 24 v1 models/covid 24 v1 Seed24.model')
